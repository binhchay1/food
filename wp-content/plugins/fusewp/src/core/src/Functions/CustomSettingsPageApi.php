...
class CustomSettingsPageApi
{
            ...
            <div class="meta-box-sortables">
                    <?php foreach ($this->sidebar_config as $arg) : ?>
                        <div class="postbox">
                            ...
                            <div class="inside">
                                <?php echo wp_kses_post($arg['content']); ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
            </div>
    ...
    private function remove_white_styling_css()
    {
        ...
        .remove_white_styling #post-body-content .fwp-email-editor-tabcontent {
            background: #fff;
            max-width: 900px;
        }

        .remove_white_styling #post-body-content .fwp-email-editor-tab {
            border: 0;
        }

        .remove_white_styling #post-body-content .fwp-email-editor-tablinks {
            background: #f1f1f1;
        }

        .remove_white_styling #post-body-content .fwp-email-editor-tablinks.eactive {
            background: #ffffff;
        }

        .remove_white_styling #post-body-content .fwp-email-editor-tabcontent.epreview {
            max-height: 650px;
        }
    ...
    public function _text_field($id, $name, $value, $class = 'regular-text', $placeholder = '', $data_key = '')
    {
        ...
        ?>
        <input type="text" placeholder="<?php echo esc_attr($placeholder); ?>" id="<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($name); ?>" class="<?php echo esc_attr($class); ?>" value="<?php echo esc_attr($value); ?>" <?php echo $data_key; ?>/>
        <?php
    ...
    */
    ...
    public function _text($db_options, $key, $args)
    {
        ...
        $class    = 'regular-text '. esc_attr(fusewpVar($args, 'class', ''));
    ...
    public function _number($db_options, $key, $args)
    {
        ...
        <tr id="<?php echo $tr_id; ?>">
            ...
            <td>
                ...
                <input type="number" id="<?php echo esc_attr($key); ?>" name="<?php echo esc_attr($option_name), '[', esc_attr($key), ']'; ?>" class="regular-text" value="<?php echo esc_attr($value); ?>" placeholder="<?php echo esc_attr($placeholder)?>"/>
    ...
    public function _password($db_options, $key, $args)
    {
        ...
        <tr id="<?php echo esc_attr($tr_id); ?>">
            ...
            <td>
                ...
                <input type="password" id="<?php echo esc_attr($key); ?>" name="<?php echo esc_attr($option_name), '[', $key, ']'; ?>"
                       class="regular-text" value="<?php echo esc_attr($value); ?>" <?php echo $disabled; ?>/>
    ...
    public function _select($db_options, $key, $args)
    {
        ...
        <tr id="<?php echo esc_attr($tr_id); ?>">
            ...
            <td>
                ...
                <select id="<?php echo esc_attr($key); ?>" name="<?php echo esc_attr($option_name), '[', $key, ']'; ?>" <?php echo esc_attr($disabled); ?><?php echo esc_attr($attributes_output)?>>
                    <?php foreach ($options as $option_key => $option_value) : ?>
                        <?php if(is_array($option_value)) : ?>
                            <optgroup label="<?php echo esc_attr($option_key)?>">
                                <?php foreach ($option_value as $key2 => $value2) : ?>
                                    <option value="<?php echo esc_attr($key2); ?>" <?php ! empty($db_options[$key]) ? selected($db_options[$key], $key2) : selected($key2, $default_select_value); ?>><?php echo esc_html($value2); ?></option>
                                <?php endforeach; ?>
                            </optgroup>
                        <?php endif;?>

                        <?php if(!is_array($option_value)) : ?>
                            <option value="<?php echo esc_attr($option_key); ?>" <?php ! empty($db_options[$key]) ? selected($db_options[$key], $option_key) : selected($option_key, $default_select_value); ?>><?php echo esc_html($option_value); ?></option>
                        <?php endif;?>
    ...
    public function _select2($db_options, $key, $args)
    {
        ...
        <tr id="<?php echo esc_attr($tr_id); ?>">
            ...
            <td>
                ...
                <input type="hidden" name="<?php echo esc_attr($option_name), '[', $key, '][]'; ?>" value="">
                <select class="wp-csa-select2" id="<?php echo esc_attr($key); ?>" name="<?php echo esc_attr($option_name), '[', $key, '][]'; ?>" <?php echo esc_attr($disabled); ?> multiple>
                    <?php foreach ($options as $option_key => $option_value) : ?>
                        <?php if(is_array($option_value)) : ?>
                            <optgroup label="<?php echo esc_attr($option_key)?>">
                                <?php foreach ($option_value as $key2 => $value2) : ?>
                                    <option value="<?php echo esc_attr($key2); ?>" <?php echo $this->select2_selected($db_options,$key,$key2,$default_select_value);?>><?php echo esc_html($value2); ?></option>
                                <?php endforeach; ?>
                            </optgroup>
                        <?php endif;?>

                        <?php if(!is_array($option_value)) : ?>
                            <option value="<?php echo esc_attr($option_key); ?>" <?php echo $this->select2_selected($db_options,$key,$option_key,$default_select_value);?>><?php echo esc_html($option_value); ?></option>
                        <?php endif;?>
    ...
    public function _checkbox($db_options, $key, $args)
    {
        ...
        <tr id="<?php echo esc_attr($tr_id); ?>">
            ...
            <td>
                ...
                <input type="hidden" name="<?php echo esc_attr($option_name), '[', $key, ']'; ?>" value="false">
                <input type="checkbox" id="<?php echo esc_attr($key); ?>" name="<?php echo esc_attr($option_name), '[', $key, ']'; ?>" value="<?php echo esc_attr($value); ?>" <?php checked($default_value, $value); ?> />
...
public function _header($args)
{
    ...
    <div class="postbox">
        ...
        <div class="postbox-header">
        <h3 class="hndle is-non-sortable"><span><?php echo $args['section_title']; ?></span></h3>
        <div class="handle-actions hide-if-no-js">
            <button type="button" class="handlediv" aria-expanded="true">
                <span class="toggle-indicator" aria-hidden="true"></span>
            </button>
    ...
    {
        return '</table>
		<p><input class="button-primary" type="submit" name="save_' . esc_attr($this->option_name) . '" value="'. esc_html__('Save Changes', 'fusewp'). '"></p>
	</div>
</div>';
    }
    ...
    public function build($exclude_sidebar = false, $exclude_top_tav_nav = false)
    {
        ...
        ?>
        <div class="wrap<?php echo $wrap_classes?>">
            ...
            <div id="poststuff" class="wp_csa_view <?php echo $this->option_name; ?><?php echo $view_classes;?>">
                ...
                <div id="post-body" class="metabox-holder<?php echo $columns2_class; ?>">
                    <div id="post-body-content">
                        ...
                        <div class="meta-box-sortables ui-sortable">
                            <form method="<?php echo  $this->form_method ?>" <?php do_action('wp_cspa_form_tag', $this->option_name); ?>>
                                <?php ob_start(); ?>
                                <?php echo $this->_settings_page_main_content_area(); ?>
                                <?php echo apply_filters('wp_cspa_main_content_area', ob_get_clean(), $this->option_name); ?>
                                <?php $this->nonce_field(); ?>
                            </form>
                        </div>

                        <?php do_action('wp_cspa_after_post_body_content', $this->option_name, $this->db_options); ?>
                    </div>
    ...
    public function build_sidebar_tab_style()
    {
        ...
        do_action('fwp_before_settings_page', $option_name);
        $nav_tabs         = '';
        $tab_content_area = '';
        ...
        if ( ! empty($settings_args)) {
            foreach ($settings_args as $key => $settings_arg) {
                ...
                $nav_tabs .= sprintf('<a href="#%1$s" class="nav-tab" id="%1$s-tab">%2$s</a>', $key, $tab_title);

                if (isset($settings_arg[0]['section_title'])) {
                    $tab_content_area .= sprintf('<div id="%s" class="fwp-group-wrapper">', $key);
            ...
            echo '<div class="wrap fwpview">';
            $this->settings_page_heading();
            $this->do_settings_errors();
            settings_errors('wp_csa_notice');
            $this->settings_page_tab();

            echo '<div class="fwp-settings-wrap" data-option-name="' . esc_attr($option_name) . '">';
            echo '<h2 class="nav-tab-wrapper">' . $nav_tabs . '</h2>';
            echo '<div class="metabox-holder fwp-tab-settings">';
            ...
            do_action('fwp_after_settings_page', $option_name);
        }
    }

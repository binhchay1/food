    ...
    setInitialServings( recipes ) {
        ...
        for ( let i = 0; i < recipes.length; i++ ) {
            ...
            if ( recipe.servings && recipe.original_servings && recipe.servings !== recipe.original_servings ) {
                ...
                setTimeout( () => {
                    window.WPRecipeMaker.manager.resetRecipe( recipe.recipe_id );
                    window.WPRecipeMaker.manager.getRecipe( recipe.recipe_id ).then( ( recipeObj ) => {
                        recipeObj.setServings( recipe.servings, recipe.id );
                    } );
                }, 100 * i );
            }
        }

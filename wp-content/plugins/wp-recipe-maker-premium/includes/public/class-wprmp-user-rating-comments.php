	...
	public static function get_ratings_for_summary_popup( $post_id, $recipe_id ) {
		...
		if ( $recipe_id && WPRM_Settings::get( 'features_user_ratings' ) ) {
			...
			if ( $ratings_to_maybe_fix ) {
				...
				if ( $mv_reviews_table === $wpdb->get_var( "SHOW TABLES LIKE '$mv_reviews_table'" ) ) {
					$reviews = $wpdb->get_results( 'SELECT * FROM ' . $mv_reviews_table . ' WHERE ID IN (' . implode( ',', $mv_review_ids ) . ')' );

					$wprm_ratings_table = WPRM_Rating_Database::get_table_name();
					if ( $wprm_ratings_table === $wpdb->get_var( "SHOW TABLES LIKE '$wprm_ratings_table'" ) ) {

 ...
 */
class WPRM_Tools_Amazon_Html_Links {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_wprm_amazon_converting_html_to_links', array( __CLASS__, 'ajax_amazon_converting_html' ) );
	}
	...
	public static function add_submenu_page() {
		add_submenu_page( '', __( 'Convert Amazon HTML to Links', 'wp-recipe-maker-premium' ), __( 'Convert Amazon HTML to Links', 'wp-recipe-maker-premium' ), WPRM_Settings::get( 'features_tools_access' ), 'wprm_amazon_html_links', array( __CLASS__, 'amazon_converting_html' ) );
	}
	...
	public static function amazon_converting_html() {
		...
		wp_localize_script( 'wprm-admin', 'wprm_tools', array(
			'action' => 'amazon_converting_html_to_links',
			'posts' => $posts,
			'args' => array(),
		));
	...
	public static function amazon_convert_html( $terms ) {
		...
		foreach ( $terms as $term_id ) {
			...
			if ( $asin_found ) {
				$link = WPRMP_Amazon::get_noapi_affiliate_link( $asin_found );
...
WPRM_Tools_Amazon_Html_Links::init();
